...
export function Sidebar({ status, availableTools, onClearHistory, onShutdown }: SidebarProps) {
  return (
    <div className="w-64 bg-[var(--bg-secondary)] border-r border-[var(--bg-tertiary)] flex flex-col h-full transition-all duration-300">
      <div className="p-6 border-b border-[var(--bg-tertiary)] bg-[var(--bg-secondary)]/50 backdrop-blur-sm">
        <div className="flex items-center gap-3 mb-1">
          <div className="w-8 h-8 bg-gradient-to-br from-[var(--accent)] to-[var(--accent-hover)] rounded-lg flex items-center justify-center text-white font-bold shadow-lg">
            B
          </div>
          <div>
      ...
      <div className="p-4 border-b border-[var(--bg-tertiary)]">
        ...
        <div className="space-y-2 bg-[var(--bg-tertiary)]/20 p-2 rounded-lg border border-[var(--bg-tertiary)]/30">
      ...
      <div className="p-4 border-t border-[var(--bg-tertiary)] bg-[var(--bg-secondary)] space-y-2">
        <button
          onClick={onClearHistory}
          disabled={!status.initialized}
          className="w-full py-2 px-3 text-xs font-medium bg-[var(--bg-tertiary)] hover:bg-[var(--bg-tertiary)]/80 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed text-[var(--text-primary)] rounded-lg transition-all flex items-center justify-center gap-2 group"
        >
        ...
        <button
          onClick={onShutdown}
          disabled={!status.initialized}
          className="w-full py-2 px-3 text-xs font-medium bg-[var(--error)]/10 hover:bg-[var(--error)] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed text-[var(--error)] rounded-lg transition-all flex items-center justify-center gap-2 group border border-[var(--error)]/20 hover:border-transparent"
        >
          <svg className="w-3.5 h-3.5 opacity-70 group-hover:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="flex items-center justify-between px-1">
      ...
      <div className="flex items-center gap-1.5">
        <span className={`w-2 h-2 rounded-full shadow-sm ${active ? 'bg-[var(--success)] animate-pulse' : 'bg-[var(--text-secondary)]/50'}`} />
